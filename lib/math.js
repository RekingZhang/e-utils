 ...
 */
(function () {
  var unboundReduce = Array.prototype.reduce;
  var reduce = Function.prototype.call.bind(unboundReduce);
  /**
   * 计算浮点数放大倍数，整数放大倍数为1
   *
  ...
  Math.add = function () {
    var maxMult = findMaxMult.apply(null, arguments);
  ...
  Math.div = function () {
    function reducer(accumulator, currentValue) {
      var maxMult = findMaxMult(accumulator, currentValue);
      return Math.mul(accumulator, maxMult) / Math.mul(currentValue, maxMult);
    }
